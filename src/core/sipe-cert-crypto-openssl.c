 ...
 *
 *     key              - NULL
 *     public           - reference to server public key, must be freed
 *     decoded          - certificate as OpenSSL data structure, must be freed
 *     raw              - NULL
				 ...
				 gsize length)
{
	...
	co->public = X509_get_pubkey(co->decoded);
	...
	co->length = EVP_PKEY_bits(co->public);
...
{
	return(((struct certificate_openssl *) certificate)->public);
}
