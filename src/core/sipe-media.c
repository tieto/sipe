...
sipe_media_sdp_candidates_format(GList *candidates, sipe_media_call* call, gboolean remote_candidate)
{
	...
	if (remote_candidate && call->remote_candidates) {
...
static void
sipe_invite_call(struct sipe_account_data *sip, TransCallback tc)
{
	gchar *hdr;
	gchar *contact;
	gchar *body;
...
static void call_hangup_cb(sipe_media_call *call, gboolean local)
{
	...
	if (local)
		send_sip_request(call->sip->gc, "BYE", call->dialog->with, call->dialog->with,
