...
- (void)configureForAccount:(AIAccount *)inAccount
{
	...
	if (room_list == NULL) {
		...
		timer = [NSTimer scheduledTimerWithTimeInterval:0.5 target:self selector:@selector(checkForRoomlistFetchCompletion:) userInfo:nil repeats:YES];
	}
...
- (void)joinChatWithAccount:(AIAccount *)inAccount
{
	...
	if (idx >= 0 && [room_dict count]) {
		...
		NSString *uri = [room_dict valueForKey:key];

		if (key && uri && [uri length]) {
			chatCreationInfo = [NSMutableDictionary dictionaryWithObjectsAndKeys:uri, @"uri", key, @"name", nil];
			[self doJoinChatWithName:[NSString stringWithFormat:@"%@", key]
						   onAccount:inAccount
		...
		// check if a URI is entered we can use to join
		NSString *uri = [combo_rooms stringValue];
